...
public class UsuarioFacade extends SuperFacade<Usuario> {
    ...
    private String monteCorpoEsqueciMinhaSenha(RecuperaSenha alteraSenha, Pessoa p) {
        FacesContext fc = FacesContext.getCurrentInstance();
        HttpServletRequest request = (HttpServletRequest) fc.getExternalContext().getRequest();
        String url = request.getRequestURL().toString();
        url = url.substring(0, url.indexOf("/faces/"));
        String corpo = " <p>Olá " + p.getNome() + ", <br> Recebemos uma solicitação de alteração da senha de sua conta no E-Procon.<br/>"
